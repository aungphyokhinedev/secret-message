...
export function Hero({ isSignedIn }: HeroProps) {
  ...
  return (
    <section className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-6 py-20 text-center md:text-left">
      <Badge
        variant="outline"
        className="mx-auto h-auto w-fit border-indigo-200 bg-indigo-50 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.2em] text-indigo-600 md:mx-0"
      >
        {t("Secret Message + Virtual Gifts", "လျှို့ဝှက်စာ + ဗာချွယ်လ် လက်ဆောင်")}
      </Badge>
      <H1 className="scroll-m-0 text-balance text-slate-900 md:max-w-3xl md:text-6xl lg:text-6xl">
        {t(
      ...
      <Lead className="mx-auto max-w-2xl text-base text-slate-600 md:mx-0 md:text-lg">
        {t(
      ...
      <div className="flex flex-col justify-center gap-3 md:flex-row md:justify-start">
        <Link
          ...
          className={cn(
            buttonVariants(),
            "rounded-full border-0 bg-indigo-500 px-6 py-3 text-sm font-semibold text-white hover:bg-indigo-400",
          )}
        >
          {isSignedIn
        ...
        <Link
          ...
          className={cn(
            ...
            "rounded-full border-indigo-200 bg-white px-6 py-3 text-sm font-semibold text-indigo-700 hover:border-indigo-300 hover:bg-white hover:text-indigo-800",
          )}
        >
